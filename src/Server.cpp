...
// reuse,sunucunun hemen kapatıldıktan sonra yeniden başlatılabilmesi için gereklidir.
void Server::socketStart()
{
	_serverSocketFD = socket(_serverSocketFamily, _serverSocketProtocol, 0); //tcp soket oluşturulur
	if (_serverSocketFD == -1)
		ErrorLogger(FAILED_SOCKET, __FILE__, __LINE__);
	// Soketi non-blocking moda ayarlar. bloklanmasını önler
	if (fcntl(_serverSocketFD, F_SETFL, O_NONBLOCK) == -1)
	{
		close(_serverSocketFD);
		ErrorLogger(FAILED_SOCKET_NONBLOCKING, __FILE__, __LINE__);
	}
	//soketin tekrar kullanılabilmesini sağlar.
	int reuse = 1;
...
//soket adresini ayarlar ve bağlar
void Server::socketInit()
{
	switch (_serverSocketFamily)
	{
		case AF_INET: // IPv4 adres ailesi
...
//bind fonksiyonu ile soketi belirlenen port numarasına bağlar
...
//listen fonksiyonu ile soketi dinlemeye alır, soketi okuma setine (read_set) ekler
...
void Server::serverRun()
{
	...
	socketBind();
	socketListen();
	signal(SIGINT, signalHandler); //ctrl+c sinyali alındığında tetiklenecek sinyal işleyici atanır, server kapatılır.
	try
	{
	...
	while (true)
	{
		...
		/*
		select fonksiyonunu kullanarak gelen bağlantıları, bottan gelen mesajları ve clientlardan gelen mesajları dinler.
		gelen bağlantı varsa socketAccept fonksiyonu çağrılır.
		clientlardan gelen mesajlar handleClient fonksiyonu ile işlenir.
		bottan veri, mesaj varsa listen fonksiyonu ile dinlenir.
		*/
		if(n)
		{
