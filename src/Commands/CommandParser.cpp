...
//strip fonksiynonu gibi çalışır, bütün whitespace karakterlerini temizler
...
// Örneğin, mergeString({"Merhaba", "Dünya"}, " ") çağrısı, "Merhaba Dünya" stringini döndürecektir.
...
//Örneğin, splitString("Merhaba,Dünya,Nasılsın", ",") çağrısı, {"Merhaba", "Dünya", "Nasılsın"} vektörünü döndürecektir.
...
void CommandParser::commandParser(const char *command, Client *client, Server *srv)
{
    ...
    handleCommand(client, commandParts, srv);//komutları işle
}
...
void CommandParser::handleCommand(Client *client, vector<string> commandParts, Server *srv)
{
    ...
    else //kullanıcı yetkilendirilmediyse veya yetkisiz bir komut girdiyse
    {
